        ...
        private void UpdateViewportTransform(GpuState state)
        {
            ...
            for (int index = 0; index < Constants.TotalViewports; index++)
            {
                ...
                if (index == 0)
                {
                    ...
                    if (!float.IsInfinity(extents.DepthNear) && !float.IsInfinity(extents.DepthFar))
                    {
                    ...
                    else
                    {
                        ...
                        depthMode = (DepthMode)(state.Get<int>(MethodOffset.DepthMode) & 1);
                    }
        ...
        private void UpdateVertexBufferState(GpuState state, int firstIndex, int indexCount)
        {
            ...
            for (int index = 0; index < Constants.TotalVertexBuffers; index++)
            {
                ...
                if (_ibStreamer.HasInlineIndexData || _drawIndexed || stride == 0 || instanced)
                {
                    ...
                    {
                        ulong vertexCount = IbUtils.GetVertexCount(state.Channel.MemoryManager, state, firstIndex, indexCount);
